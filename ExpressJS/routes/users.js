...
// Third-party API integration route (public)
// GET /api/users/external-posts
router.get('/external-posts', async (req, res, next) => {
  const now = Date.now();
  if (cachedPosts && now - cacheTime < 60000) { // 1 minute cache
